...
{
    public class UserProfileRawEventsRpcConsumer : BackgroundService
    {
        private readonly IServiceScopeFactory _scopeFactory;
        private readonly IConfiguration _configuration;
        private readonly ILogger<UserProfileRawEventsRpcConsumer> _logger;

        private IConnection? _connection;
        private IChannel? _channel;

        public UserProfileRawEventsRpcConsumer(
            IServiceScopeFactory scopeFactory,
            IConfiguration configuration,
            ILogger<UserProfileRawEventsRpcConsumer> logger)
        {
        ...
        public override async Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            await _channel.QueueDeclareAsync(
                queue: "profile.raw-events.request",
                durable: false,
                exclusive: false,
                autoDelete: false,
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            consumer.ReceivedAsync += async (_, ea) =>
            {
                ...
                {
                    _logger.LogError(ex, "Profile raw events RPC failed");
                }
